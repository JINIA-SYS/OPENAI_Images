...
const Home = () => {
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            setLoading(true)
            try {
                const response = await fetch("http://localhost:8080/api/v1/post", { method: 'GET', headers: { 'Content-Type': 'application/json' } })
                if (response.ok) {
    ...
    return (
        <section className='max-w-7xl mx-auto'>
            ...
            <div className='mt-16 '>
                <FormField labelName="SearchPosts" type="text" name="text" placeholder="SearchPosts" value={searchText} handleChange={handleSearchChange} />
            ...
            <div className='mt-10'>
                {loading ?
                    ...
                    (<> {searchText && (<h2 className='font-medium text-[#666e75] text-xl mb-3'>
                        Showing result for <span className='text-[#222328]'>{searchText}  </span> </h2>
                    )}
                        <div className='grid lg:grid-cols-4 sm:grid-cols-3 xs:grid-cols-2 grid-cols-1 gap-3'>
                            {searchText ? (<Rendercards data={searchedResults} title="No results found" />) : (<Rendercards data={allPosts} title="No Posts found" />)}
                        </div>
